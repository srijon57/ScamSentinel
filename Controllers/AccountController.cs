...
public class AccountController : Controller
{
    ...
    public AccountController(SupabaseService supabaseService, EmailService emailService, IConfiguration configuration, CloudinaryService cloudinaryService, IWebHostEnvironment env)
    {
    ...
    private CookieOptions BuildAccessTokenOptions()
    {
        return new CookieOptions
        {
            ...
            Expires = DateTime.UtcNow.AddMinutes(Convert.ToDouble(_configuration["Jwt:AccessTokenMinutes"]))
        };
    }
    ...
    private CookieOptions BuildRefreshTokenOptions()
    {
        return new CookieOptions
        {
            ...
            Secure = false,           
            SameSite = SameSiteMode.Lax,
            Expires = DateTime.UtcNow.AddDays(Convert.ToDouble(_configuration["Jwt:RefreshTokenDays"]))
        };
    }
    ...
    public async Task<IActionResult> Register(RegisterModel model)
    {
        ...
        try
        {
            ...
            {
                TempData["ErrorMessage"] = existingUser.UserName.Equals(model.UserName, StringComparison.OrdinalIgnoreCase)
    ...
    public async Task<IActionResult> VerifyOTP(VerifyOTPModel model)
    {
        try
        {
            ...
            await connection.ExecuteAsync("UPDATE OTP SET IsUsed = TRUE WHERE OTPID = @OTPID", new { otp.OTPID });
            await connection.ExecuteAsync("UPDATE Users SET IsVerified = TRUE WHERE Email = @Email", new { model.Email });
            ...
            Response.Cookies.Append("access_token", accessToken, BuildAccessTokenOptions());
            Response.Cookies.Append("refresh_token", refreshToken, BuildRefreshTokenOptions());
    ...
    private string GenerateJwtToken(User user)
    {
        ...
        var token = new JwtSecurityToken(
            ...
            claims: claims,
            expires: DateTime.Now.AddMinutes(Convert.ToDouble(_configuration["Jwt:AccessTokenMinutes"])),
